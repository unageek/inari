...
use inari::{
    const_dec_interval, const_interval, dec_interval, interval, DecoratedInterval, Decoration,
    Interval,
};
use std::{
    collections::HashMap,
...
impl Region {
    fn new(l: f64, r: f64, b: f64, t: f64) -> Self {
...
struct GraphingError {
    kind: GraphingErrorKind,
}
...
#[derive(Debug)]
struct Relation(fn(TupperIntervalSet, TupperIntervalSet) -> EvaluationResult);
...
impl Graph {
    ...
    fn new(relation: Relation, region: Region, im_width: u32, im_height: u32) -> Self {
    ...
    fn step(&mut self) -> Result<bool, GraphingError> {
...
fn f(x: TupperIntervalSet, y: TupperIntervalSet) -> EvaluationResult {
    ...
    //   ❗: Have a serious problem with graphing
    //   🐌: Very slow to finish
    ...
    //let c0 = TupperIntervalSet::from(C0);
    //let xy = &x * &y;
    //let a = x.div(&x.sin(), Some(0));
    //let b = y.div(&y.sin(), Some(1));
    //let c = xy.div(&xy.sin(), Some(2));
    //let apb = &a + &b;
    //let amb = &a - &b;
    //(&(&(&apb + &c) * &(&apb - &c)) * &(&(&amb + &c) * &(&amb - &c))).eq(&c0)
