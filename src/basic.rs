...
impl Interval {
    ...
    pub fn mul_rev_to_pair(self, numerator: Self) -> (Self, Self) {
        ...
        match numerator.classify2(self) {
            E_E | E_M | E_N0 | E_N1 | E_P0 | E_P1 | E_Z | M_E | N0_E |  N1_E
                | N1_Z | P0_E | P1_E | P1_Z | Z_E =>
                (Self::EMPTY, Self::EMPTY),
            M_Z | N0_Z | P0_Z | Z_Z | M_M | M_N0 | M_P0 | N0_M | P0_M | Z_M
                | Z_N0 | Z_P0 | N0_N0 | N0_P0 | P0_N0 | P0_P0  =>
            ...
            N1_M => {
                ...
                (Self { rep: shuffle13(splat(f64::INFINITY), q)},
                 Self { rep: shuffle02(q, splat(f64::INFINITY)) })
            }
            P1_M => {
                ...
                (Self { rep: shuffle02(splat(f64::INFINITY), q) },
                 Self { rep: shuffle13(q, splat(f64::INFINITY)) })
            }
            // Below are the common rules with `numerator`/`self`
            M_N1 => {
                // M / N1 => [b/d, a/d] = [-b/d; a/d] = [b/-d; -a/-d]  = [b; -a] ./ [-d; -d]
                let x = swap(numerator.rep); // [b; -a]
                let y = swap(self.rep); // [d; -c]
                let y = neg0(y); // [-d; -c]
                let y = shuffle02(y, y); // [-d; -d]
                (Self { rep: div_ru(x, y) }, Self::EMPTY)
            }
            M_P1 => {
                // M / P1 => [a/c, b/c] = [-a/c; b/c] = [-a; b] ./ [c; c]
                let x = numerator.rep; // [-a; b]
                let y = neg0(self.rep); // [c; d]
            ...
            N1_N0 => {
                ...
                let x = swap(numerator.rep); // [b; -a]
                let y = self.rep; // [-c; d]
                (Self { rep: shuffle02(div_ru(x, y), splat(f64::INFINITY)) },
                 Self::EMPTY)
            }
            N0_N1 | N1_N1 => {
                // N / N1 => [b/c, a/d] = [-b/c; a/d] = [b/-c; a/d] = [b; a] ./ [-c; d]
                let x = neg0(numerator.rep); // [a; b]
                let x = swap(x); // [b; a]
                let y = self.rep; // [-c; d]
                (Self { rep: div_ru(x, y) }, Self::EMPTY)
            }
            N1_P0 => {
                ...
                let x = numerator.rep; // [-a; b]
                let y = self.rep; // [-c; d]
                (Self { rep: shuffle03(splat(f64::INFINITY), div_ru(x, y)) },
                 Self::EMPTY)
            }
            N0_P1 | N1_P1 => {
            ...
            P1_N0 => {
                ...
                let x = numerator.rep; // [-a; b]
                let y = self.rep; // [-c; d]
                (Self { rep: shuffle02(splat(f64::INFINITY), div_ru(x, y)) },
                 Self::EMPTY)
            }
            P0_N1 | P1_N1 => {
...
mod tests {
    ...
    fn mul_rev_to_pair() {
        let zero = const_interval!(0., 0.);
        assert_eq!(zero.mul_rev_to_pair(const_interval!(1., 2.)),
                   (I::EMPTY, I::EMPTY));
        let pos = const_interval!(0., f64::INFINITY);
        let neg = const_interval!(f64::NEG_INFINITY, 0.);
        assert_eq!(I::ENTIRE.mul_rev_to_pair(const_interval!(1., 1.)),
                   (neg, pos));
