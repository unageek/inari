...
fn ieee1788_b() {
    ...
    assert_eq!(t2di("[1,1e3]_com"), nd2di(1.0, 1000.0, D::Com));
    assert_eq!(t2di("[1,1E3]_COM"), nd2di(1.0, 1000.0, D::Com));
}
...
fn ieee1788_e2() {
    ...
    assert_eq!(t2di("[0x1.3p-1,]"), nd2di(hexf64!("0x1.3000000000000p-1"), f64::INFINITY, D::Dac));
    assert_eq!(t2di("[,]"), nd2di(f64::NEG_INFINITY, f64::INFINITY, D::Dac));
    assert_eq!(t2di("3.56?1"), nd2di(hexf64!("0x1.c666666666666p+1"), hexf64!("0x1.c8f5c28f5c290p+1"), D::Com));
    assert_eq!(t2di("3.56?1e2"), nd2di(355.0, 357.0, D::Com));
    assert_eq!(t2di("3.560?2"), nd2di(hexf64!("0x1.c76c8b4395810p+1"), hexf64!("0x1.c7ef9db22d0e6p+1"), D::Com));
    assert_eq!(t2di("3.56?"), nd2di(hexf64!("0x1.c70a3d70a3d70p+1"), hexf64!("0x1.c851eb851eb86p+1"), D::Com));
    assert_eq!(t2di("3.560?2u"), nd2di(hexf64!("0x1.c7ae147ae147ap+1"), hexf64!("0x1.c7ef9db22d0e6p+1"), D::Com));
    assert_eq!(t2di("-10?"), nd2di(-10.5, -9.5, D::Com));
    assert_eq!(t2di("-10?u"), nd2di(-10.0, -9.5, D::Com));
    assert_eq!(t2di("-10?12"), nd2di(-22.0, 2.0, D::Com));
    assert_eq!(t2di("-10??u"), nd2di(-10.0, f64::INFINITY, D::Dac));
    assert_eq!(t2di("-10??"), nd2di(f64::NEG_INFINITY, f64::INFINITY, D::Dac));
    assert!(t2di("[nai]").is_nai());
    assert_eq!(t2di("3.56?1_def"), nd2di(hexf64!("0x1.c666666666666p+1"), hexf64!("0x1.c8f5c28f5c290p+1"), D::Def));
}
