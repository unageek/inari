...
fn minimal_overlap_test() {
    ...
    assert_eq2!(
        n2i(f64::NEG_INFINITY, 2.0).overlap(n2i(3.0, f64::INFINITY)),
        O::Before
    );
    assert_eq2!(
        n2i(f64::NEG_INFINITY, 2.0).overlap(n2i(3.0, 4.0)),
        O::Before
    );
    ...
    assert_eq2!(
        n2i(1.0, 2.0).overlap(n2i(f64::NEG_INFINITY, 3.0)),
        O::ContainedBy
    );
    assert_eq2!(n2i(1.0, 2.0).overlap(n2i(0.0, 3.0)), O::ContainedBy);
    assert_eq2!(n2i(2.0, 2.0).overlap(n2i(0.0, 3.0)), O::ContainedBy);
    assert_eq2!(
        n2i(2.0, 2.0).overlap(n2i(0.0, f64::INFINITY)),
        O::ContainedBy
    );
    assert_eq2!(
        n2i(1.0, 2.0).overlap(n2i(f64::NEG_INFINITY, 2.0)),
        O::Finishes
    );
    ...
    assert_eq2!(
        n2i(f64::NEG_INFINITY, 1.0).overlap(n2i(f64::NEG_INFINITY, 1.0)),
        O::Equals
    );
    ...
    assert_eq2!(
        n2i(1.5, 2.5).overlap(n2i(f64::NEG_INFINITY, 2.0)),
        O::OverlappedBy
    );
    ...
    assert_eq2!(
        n2i(f64::NEG_INFINITY, 3.0).overlap(n2i(1.0, 2.0)),
        O::Contains
    );
    ...
    assert_eq2!(
        n2i(f64::NEG_INFINITY, 2.0).overlap(n2i(1.0, 2.0)),
        O::FinishedBy
    );
...
fn minimal_overlap_dec_test() {
    ...
    assert_eq2!(
        DI::EMPTY.overlap(nd2di(1.0, 2.0, D::Com)).unwrap(),
        O::FirstEmpty
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Def).overlap(DI::EMPTY).unwrap(),
        O::SecondEmpty
    );
    assert_eq2!(
        nd2di(2.0, 2.0, D::Def)
            .overlap(nd2di(3.0, 4.0, D::Def))
            .unwrap(),
        O::Before
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Dac)
            .overlap(nd2di(3.0, 4.0, D::Com))
            .unwrap(),
        O::Before
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Com)
            .overlap(nd2di(3.0, 3.0, D::Trv))
            .unwrap(),
        O::Before
    );
    assert_eq2!(
        nd2di(2.0, 2.0, D::Trv)
            .overlap(nd2di(3.0, 3.0, D::Def))
            .unwrap(),
        O::Before
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Def)
            .overlap(nd2di(2.0, 3.0, D::Def))
            .unwrap(),
        O::Meets
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Dac)
            .overlap(nd2di(1.5, 2.5, D::Def))
            .unwrap(),
        O::Overlaps
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Def)
            .overlap(nd2di(1.0, 3.0, D::Com))
            .unwrap(),
        O::Starts
    );
    assert_eq2!(
        nd2di(1.0, 1.0, D::Trv)
            .overlap(nd2di(1.0, 3.0, D::Def))
            .unwrap(),
        O::Starts
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Def)
            .overlap(nd2di(0.0, 3.0, D::Dac))
            .unwrap(),
        O::ContainedBy
    );
    assert_eq2!(
        nd2di(2.0, 2.0, D::Trv)
            .overlap(nd2di(0.0, 3.0, D::Def))
            .unwrap(),
        O::ContainedBy
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Trv)
            .overlap(nd2di(0.0, 2.0, D::Com))
            .unwrap(),
        O::Finishes
    );
    assert_eq2!(
        nd2di(2.0, 2.0, D::Def)
            .overlap(nd2di(0.0, 2.0, D::Dac))
            .unwrap(),
        O::Finishes
    );
    assert_eq2!(
        nd2di(1.0, 2.0, D::Def)
            .overlap(nd2di(1.0, 2.0, D::Def))
            .unwrap(),
        O::Equals
    );
    assert_eq2!(
        nd2di(1.0, 1.0, D::Dac)
            .overlap(nd2di(1.0, 1.0, D::Dac))
            .unwrap(),
        O::Equals
    );
    assert_eq2!(
        nd2di(3.0, 4.0, D::Trv)
            .overlap(nd2di(2.0, 2.0, D::Trv))
            .unwrap(),
        O::After
    );
    assert_eq2!(
    ...
    assert_eq2!(
        nd2di(2.0, 3.0, D::Def)
            .overlap(nd2di(1.0, 2.0, D::Trv))
            .unwrap(),
        O::MetBy
    );
    assert_eq2!(
        nd2di(1.5, 2.5, D::Com)
            .overlap(nd2di(1.0, 2.0, D::Com))
            .unwrap(),
        O::OverlappedBy
    );
    assert_eq2!(
